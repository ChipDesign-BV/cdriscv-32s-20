...
> | fault injection, FMEDA | **re-measured on this design** — eight campaigns on the final RTL; FMEDA **SPFM 99.52 %, LFM 93.63 %, residual 1.18 FIT** on *assumed* base rates, populations from the final `chip2b` netlist, every row measured — the loader included since finding 22 ([doc/fmeda.md](doc/fmeda.md)) |
...
> | gate-level simulation (O8) | **met on the final `chip2b` post-route netlist, 2026-09-13/14** — the flat chip with SDF cell delays boots through the pads and the real QSPI loader, 1-bit and quad, at the typ and slow (signoff) corners, cycle-identical to RTL; twelve arch tests at the slow corner, **12 of 12 signatures match Spike** — findings §21 |
>
> What keeps this a warning: the FMEDA's base failure rates
...
| FMEDA (O9) | **computed on this design, every row measured** | **SPFM 99.52 %, LFM 93.63 %, residual 1.18 FIT** — past the ASIL D thresholds *on assumed base rates*; populations re-read from the final `chip2b` netlist (7 538 flops, row sum asserted) and diagnostic coverage from this variant's campaigns ([doc/fmeda.md](doc/fmeda.md)). The QSPI loader row was the last argued one and is now swept (`make fi-loader`, 1 956 upsets): **no reading of it fails the thresholds** — charging the whole campaign to it, load window included, still gives LFM 93.28 %. The sweep did find a real architectural gap in the loader, written up as [§22](doc/verification_findings_20.md) |
| Gate-level simulation (O8) | **met on `chip2b`, typ and slow corners** | `make -f Makefile -f verif/gate/gate_chip.mk gate-chip-sdf gate-chip-arch [CHIP_CORNER=…]`: the flat post-route chip netlist (335 518 instances, pads and SRAM macros included) with the OpenSTA SDF — 90 317 cell blocks of IOPATH delays; INTERCONNECT and macro blocks dropped for Icarus, counted, findings §21 — boots the smoke firmware through the QSPI pads at typ and at the slow signoff corner: 1-bit **PASS 11 334 cycles**, quad **PASS 3 726 cycles**, cycle-identical to RTL; **12 arch tests at the slow corner, 12 of 12 signatures match Spike** (~1–2 h wall each, annotation-bound). Checks function under annotated cell delays; timing margins are OpenSTA's |
...
the complete RTL (`chip2b`), the FMEDA counts that netlist and O8 is
met on it at both signoff corners, and what remains is the
